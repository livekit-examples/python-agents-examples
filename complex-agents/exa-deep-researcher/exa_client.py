    ...
    async def search_and_contents(
        ...
        for attempt in range(retry_count):
            try:
                ...
                response = await loop.run_in_executor(
                    None,
                    lambda: self.client.search_and_contents(params.query, **kwargs)
                )
                ...
                logger.error(f"EXA search_and_contents error (attempt {attempt + 1}/{retry_count}): {e}")
                if attempt == retry_count - 1:
                    raise
                await asyncio.sleep(backoff_factor ** attempt)
    ...
    async def find_similar(
        ...
        for attempt in range(retry_count):
            try:
                ...
                response = await loop.run_in_executor(
                    None,
                    lambda: self.client.find_similar(url, num_results=num_results)
                )
                ...
                logger.error(f"EXA find_similar error (attempt {attempt + 1}/{retry_count}): {e}")
                if attempt == retry_count - 1:
                    raise
                await asyncio.sleep(backoff_factor ** attempt)
