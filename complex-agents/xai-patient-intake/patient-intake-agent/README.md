...
export UV_PROJECT_ENVIRONMENT=src/.venv   # PowerShell: $env:UV_PROJECT_ENVIRONMENT = "src/.venv"
uv sync
uv run python src/agent.py console
uv run python src/agent.py dev
```

The environment lives at `src/.venv`. uv has no project-file setting for this, so
`UV_PROJECT_ENVIRONMENT` must be set in every shell — without it, `uv run` quietly
builds a second environment at the project root and uses that instead.
...
The unit tests assert the exact eight tools assembled in production.
