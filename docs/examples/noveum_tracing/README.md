...
calls, conversation history, and (optionally) full-conversation audio, and exports
them to [Noveum](https://noveum.ai).

## Prerequisites

- Add a `.env` in this directory with your LiveKit and Noveum credentials:
  ```
...
- `setup_livekit_tracing(session, record=True, trace_name_prefix="livekit-example")`
  attaches tracing to the `AgentSession`; every turn, STT/TTS/LLM step, and tool call
  is exported as a trace.
- **Privacy note:** `record=True` captures full conversation audio. Pass
  `record=False` for privacy-sensitive deployments; text/transcript capture is
  likewise configurable.
