...
from livekit.agents.voice import Agent, AgentSession, inference, RunContext
...
DEFAULT_TASK = "talk to a human customer service agent"
```

## Creating a data model

Create a data class to store user data and state (including the `JobContext` so tools can reach the room connection):
...
async def entrypoint(ctx: JobContext):
    ...
    def on_participant_connected(participant: rtc.RemoteParticipant):
        ...
        # Get the task from attributes (default if none was provided)
        task = participant._info.attributes.get("task") or DEFAULT_TASK
...
1. The agent is initialized with instructions to act as a human caller with a specific goal
1. The agent listens to the IVR system as it presents options
1. When the agent needs to select an option, it uses the `send_dtmf_code` function to send a DTMF tone
1. A cooldown mechanism prevents sending multiple DTMF codes too quickly
1. The agent continues to navigate through the IVR system until it accomplishes its task
